...
class Conv1dExtractor(nn.Module):
    def __init__(self, config):
        ...
        Returns:
            outputs: Tensor (batch_size, src_len, seq_len, extractor_size)
        """
        super().__init__()
        ...
        self.linear = nn.Linear(config["channels3"], config["extractor_size"])
    ...
    def forward(self, x: torch.Tensor):
        ...
        for i in range(src_len):
            ...
            out = torch.relu(self.dropout(self.linear(out3))).unsqueeze(1)

            extracted.append(out)

        extracted = torch.cat(extracted, dim=1)
...
class SpatialTemporalLayer(nn.Module):
    ...
    def pooling(
        ...
        """
        Args:
            features: Tensor (batch_size, seq_len, src_len, n_features)
        ...
        out = features.new_zeros(batch_size, features.size(1), features.size(3))
        ...
        for i in range(batch_size):
            ...
            out[i] = (features[i, :, __src_masks] * torch.softmax(inv_dists, dim=0)).sum(1)
    ...
    def forward(
        ...
        state = self._init_state(x.size(0), self.hidden_size, self.num_layers, x.device)
        # x.shape == (batch_size, seq_len, n_features)
        x = self.pooling(x.transpose(1, 2), src_locs, tar_loc, src_masks)
    ...
    def _init_state(
        ...
        ht = torch.zeros(2 * num_layers, batch_size, hidden_size, device=device)
        ct = torch.zeros(2 * num_layers, batch_size, hidden_size, device=device)
